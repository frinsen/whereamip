...
# One-time setup: brew install --cask cloudflare-warp && warp-cli registration new
E2E_EXPECTS_VPN=1
...
e2e_available() {
  ...
  warp-cli --accept-tos status >/dev/null 2>&1 || { echo "warp-cli present but not registered (warp-cli registration new)"; return 1; }
}
