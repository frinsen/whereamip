...
<p align="center">
  <img src="docs/screenshot.png" width="860" alt="WhereAmIP with a VPN active: the Argentine flag with a warning badge in the menu bar, and the dropdown showing an IPv6 leak warning, the exit in Buenos Aires via PureVPN, and the DNS row">
</p>
...
- Quiet update hint — checks the latest GitHub release a few times a day (at most once every six hours, riding along with a refresh) and whenever you hit Refresh; when one's out, a single dropdown row offers the update. The row adapts to how you installed WhereAmIP: it copies `brew update && brew upgrade whereamip` for a Homebrew install (the `brew update` half matters — see the FAQ), `sudo port selfupdate && sudo port upgrade whereamip` for a MacPorts one, and opens the releases page if you installed from the zip, where there is no command to run. Once the upgrade has landed, the row becomes "↻ Restart to finish update" — one click relaunches into the new version. No popups, no badges, respects your Settings, and the app itself never downloads or self-updates — your package manager does
- One-time welcome window on first start (and once after major releases): what the app is, where it lives, and live toggles for Launch at Login, Add to Applications folder, and Show Notifications — no permission prompts unless you flip a toggle. Reopen it anytime via **Settings ▸ Show Welcome Window**, and read the current release's highlights on demand via **Settings ▸ What's New** — both are explicit, neither depends on what you have already seen
...
That is exactly what the dropdown's update row copies for you — for a MacPorts install it copies `sudo port selfupdate && sudo port upgrade whereamip` instead, for the same reason: an unsynced ports tree has no newer version to find.
...
WhereAmIP also checks `api.github.com/repos/frinsen/whereamip/releases/latest` a few times a day (at most once every six hours, plus whenever you hit Refresh) to see if a newer release exists — this is a passive version check only, it never downloads or installs anything, it just shows a dropdown row that copies your package manager's upgrade command for you to run yourself (or, for a zip install, opens the releases page in your browser when you click it). This check is on by default; turn it off with `whereamip config set updates false` (or the "Check for Updates" toggle in Settings) and no such request is ever made.
...
- An iOS companion app, coupled to the notarization decision above (same Apple Developer membership unlocks both doors). It would be a different product with a weaker promise: most of the Core — exit lookups, Private Relay detection, DNS egress enumeration, the diagnostics report — ports cleanly, but iOS has no menu bar, no always-running agent (so "the flag flips the moment a VPN takes the route" is impossible — a widget shows the last check with an honest timestamp), and none of the introspection APIs behind VPN naming and hijack detection. An honest iOS WhereAmIP is a check-now tool with widgets and Shortcuts integration, taken as a second harvest if the membership ever happens — never the reason for it.
